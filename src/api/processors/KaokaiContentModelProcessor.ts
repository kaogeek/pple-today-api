    ...
    public process(): Promise<SectionModel> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let filterContentsHashTag = undefined;
                ...
                if (this.data !== undefined && this.data !== null) {
                    ...
                    filterContentsHashTag = this.data.filterContentsHashTag;
                ...
                if (filterContentsHashTag.length > 0) {
                    for (const contents of filterContentsHashTag) {
                        postId.push(new ObjectID(contents.post._id));
                    }
                }
                ...
                if (ContentProcessor.type === 'post' && ContentProcessor.field === 'score') {
                    ...
                    if (ContentProcessor.buckets.length >= 0) {
                        for (const pageGroups of ContentProcessor.buckets) {
                    ...
                    const stackPage = [];
                    const switchSort = ContentProcessor.flag;
                    ...
                    result.dateTime = lastestDate;
                    resolve(result);
                } else if (ContentProcessor.type === 'post' && ContentProcessor.field === 'hashtag') {
                    const bucketSAll = [];
                    const postAggregateAll = [];
                    const chunkSizes = [];
                    if (ContentProcessor.buckets.length >= 0) {
                        for (const hashTags of ContentProcessor.buckets) {
                    ...
                    const bucketAll = [];
                    if (ContentProcessor.buckets.length >= 0) {
                        for (const provinceAll of ContentProcessor.buckets) {
                    ...
                    const chunkSizes = [];
                    if (ContentProcessor.buckets.length >= 0) {
                        for (const IdAll of ContentProcessor.buckets) {
                    ...
                    const result: SectionModel = new SectionModel();
                    result.title = (this.config === undefined || this.config.title === undefined) ? ContentProcessor.title : 'เกาะกระแส';
                    result.subtitle = '';
                    result.description = '';
                    result.iconUrl = '';
                    result.contents = [];
